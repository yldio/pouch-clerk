...
Databases.prototype._listenTo = function _listenTo(db) {
  debug('going to listen to changes on %', db.name);
  var self = this;
  ...
  function onChange(change) {
    ...
    db.db.get(change.id, function(err, doc) {
      if (rev === doc._rev) {
        debug('revision matches');
        self.emit('change', doc);
      } else {
        debug('revision does not match: %j and %j', rev, doc._rev);
      }
    });
  }
...
Databases.prototype.remove = function remove(db) {
  var name;
  if (typeof db == 'string') {
    name = db;
  } else {
  ...
  var db = name && this._databases[name];
  if (db) {
